        ...
        {!showHistory ? (
          <>
            ...
            {favoriteProducts.length === 0 ? (
              <div className="text-center py-8 text-gray-500"><p className="mb-2">⭐ お気に入り商品がありません</p><p className="text-sm">下の「お気に入り設定」から追加してください</p></div>
            ) : (
              <div className="space-y-2 mb-4">
                {favoriteProducts.map(p => (
                  <div key={p.id} className={`flex justify-between items-center p-3 rounded border ${entries[p.id] > 0 ? 'bg-blue-50 border-blue-300' : 'bg-white border-gray-200'}`}>
                    <div><div className="font-semibold">{p.name}</div><div className="text-sm text-gray-500">{p.largeCategory} / {p.mediumCategory}</div></div>
                    <div className="flex items-center gap-2">
                      ...
                      <table className="text-sm">
                        ...
                        <tbody>{filtered.map(p => { const q = inv[p.id] || 0; const last = getLastInventory(p.id); const usageQty = getUsageSinceLastInventory(p.id); const stockInQty = getStockInSinceLastInventory(p.id); const expected = getExpectedInventory(p.id); const diff = getDifference(p.id); return (
                          <tr key={p.id} style={{ background: diff !== null && diff !== 0 ? '#fefce8' : '' }}>
                            ...
                            <td className="text-center"><input type="number" value={q} onChange={e => setInv({...inv, [p.id]: parseInt(e.target.value) || 0})} className="input" style={{ width: '5rem', textAlign: 'center' }} min="0" /></td>
...
function StaffPurchase({ products, staff, staffPurchases, setStaffPurchases }) {
  ...
  const recordPurchase = async () => {
    ...
    if (!product) return
    const { data, error } = await supabase.from('staff_purchases').insert({ staff_name: selectedStaff, product_id: product.id, product_name: product.name, large_category: product.largeCategory, medium_category: product.mediumCategory, purchase_price: product.purchasePrice, quantity, purchase_date: date }).select()
    if (!error && data) { setStaffPurchases([...staffPurchases, { id: data[0].id, staff: selectedStaff, productId: product.id, productName: product.name, largeCategory: product.largeCategory, mediumCategory: product.mediumCategory, purchasePrice: product.purchasePrice, quantity, date }]); alert('購入を記録しました！'); setQuantity(1) }
  }
  ...
  const startEdit = (record) => { setEditingId(record.id); setEditData({ staff: record.staff, quantity: record.quantity, date: record.date }) }
  const saveEdit = async (id) => { const { error } = await supabase.from('staff_purchases').update({ staff_name: editData.staff, quantity: parseInt(editData.quantity) || 1, purchase_date: editData.date }).eq('id', id); if (!error) { setStaffPurchases(staffPurchases.map(p => p.id === id ? { ...p, staff: editData.staff, quantity: parseInt(editData.quantity) || 1, date: editData.date } : p)); setEditingId(null) } }

  const monthlyPurchases = staffPurchases.filter(p => p.date?.startsWith(selectedMonth))
  const staffSummary = {}; monthlyPurchases.forEach(p => { if (!staffSummary[p.staff]) staffSummary[p.staff] = { items: [], total: 0 }; staffSummary[p.staff].items.push(p); staffSummary[p.staff].total += p.purchasePrice * p.quantity })
  ...
  return (
    <div className="space-y-4">
      <div className="card">
        ...
        <div className="grid-2 mb-4">
          <div><label className="text-sm font-semibold mb-2" style={{ display: 'block' }}>商品</label><select value={selectedProduct} onChange={e => setSelectedProduct(e.target.value)} className="select"><option value="">選択</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}（¥{p.purchasePrice.toLocaleString()}）</option>)}</select></div>
        ...
        {Object.keys(staffSummary).length === 0 ? (<p className="text-gray-500 text-center py-4">この月の購入記録はありません</p>) : (
          Object.entries(staffSummary).map(([staffName, data]) => (
            <div key={staffName} className="mb-4 border rounded-lg overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="text-sm"><thead><tr><th>日付</th><th>商品</th><th className="text-center">数量</th><th className="text-right">金額</th><th className="text-center">操作</th></tr></thead>
                  <tbody>
                    {data.items.map(item => (
                      editingId === item.id ? (
                        <tr key={item.id} style={{ background: '#fef9c3' }}>
                          ...
                          <td>{item.productName}</td><td className="text-center"><input type="number" value={editData.quantity} onChange={e => setEditData({...editData, quantity: e.target.value})} className="input" style={{ width: '60px' }} min="1" /></td>
                          <td className="text-right">¥{(item.purchasePrice * (parseInt(editData.quantity) || 1)).toLocaleString()}</td>
                          <td className="text-center"><button onClick={() => saveEdit(item.id)} className="text-green-600 text-sm mr-2">保存</button><button onClick={() => setEditingId(null)} className="text-gray-500 text-sm">取消</button></td>
                        </tr>
                      ) : (
                        <tr key={item.id}><td>{item.date}</td><td>{item.productName}</td><td className="text-center">{item.quantity}</td><td className="text-right">¥{(item.purchasePrice * item.quantity).toLocaleString()}</td>
                          <td className="text-center"><button onClick={() => startEdit(item)} className="text-blue-500 text-sm mr-2">編集</button><button onClick={() => deletePurchase(item.id)} className="text-red-500 text-sm">削除</button></td>
                        </tr>
                      )
                    ))}
                  </tbody>
...
function StaffManagement({ staff, setStaff, categories }) {
  ...
  const addStaff = async () => {
    ...
    const { data, error } = await supabase.from('staff').insert({ name: newStaff, dealer: dealerStr, join_date: newJoinDate || null, tenure_rate: newTenureRate, work_type: newWorkType, part_time_rate: newPartTimeRate, is_opening_staff: newIsOpening, special_rate: newSpecialRate }).select()
    if (!error && data) {
      setStaff([...staff, { id: data[0].id, name: newStaff, dealer: dealerStr, joinDate: newJoinDate || null, tenureRate: newTenureRate, workType: newWorkType, partTimeRate: newPartTimeRate, isOpeningStaff: newIsOpening, specialRate: newSpecialRate }])
      setNewStaff(''); setNewDealers([]); setNewJoinDate(''); setNewTenureRate(100); setNewWorkType('full'); setNewPartTimeRate(100); setNewIsOpening(false); setNewSpecialRate(0)
    }
  }
  ...
  const startEdit = (s) => { setEditingId(s.id); setEditData({ name: s.name, dealers: s.dealer ? s.dealer.split(',').filter(d => d) : [], joinDate: s.joinDate || '', tenureRate: s.tenureRate || 100, workType: s.workType || 'full', partTimeRate: s.partTimeRate || 100, isOpeningStaff: s.isOpeningStaff || false, specialRate: s.specialRate || 0 }) }
  const saveEdit = async (id) => {
    const dealerStr = editData.dealers.join(',')
    const { error } = await supabase.from('staff').update({ name: editData.name, dealer: dealerStr, join_date: editData.joinDate || null, tenure_rate: editData.tenureRate, work_type: editData.workType, part_time_rate: editData.partTimeRate, is_opening_staff: editData.isOpeningStaff, special_rate: editData.specialRate }).eq('id', id)
...
// ==================== ボーナス原資管理（管理者のみ） ====================
function BonusManagement({ staff, bonusSettings, setBonusSettings, stockIn, products }) {
  ...
  const calcActualPurchase = (start, end) => {
  ...
  const savePeriod = async () => {
    if (!periodStart || !periodEnd) { alert('期間を設定してください'); return }
    const actualPurchase = calcActualPurchase(periodStart, periodEnd)
    const { data, error } = await supabase.from('bonus_settings').insert({ period_start: periodStart, period_end: periodEnd, target_sales: parseInt(targetSales) || 0, target_rate: parseFloat(targetRate) || 20, actual_purchase: actualPurchase, memo: '' }).select()
    if (!error && data) {
      setBonusSettings([...bonusSettings, { id: data[0].id, periodStart, periodEnd, targetSales: parseInt(targetSales) || 0, targetRate: parseFloat(targetRate) || 20, actualPurchase, memo: '' }])
      alert('保存しました！'); setPeriodStart(''); setPeriodEnd(''); setTargetSales(''); setTargetRate('20')
    }
  }
  ...
  const calcBonus = (setting) => {
    const targetBudget = Math.round(setting.targetSales * setting.targetRate / 100)
    const savings = targetBudget - setting.actualPurchase
    return savings > 0 ? savings : 0
  }

  const calcDistribution = (setting) => {
    ...
    return staffCoefs.map(s => ({ ...s, share: totalCoef > 0 ? Math.round(bonus * s.coef / totalCoef) : 0 }))
  }
  ...
  return (
    <div className="space-y-4">
      <div className="card">
        ...
        <div className="grid-2 mb-4">
          <div><label className="text-sm font-semibold mb-1" style={{ display: 'block' }}>売上目標</label><input type="number" value={targetSales} onChange={e => setTargetSales(e.target.value)} placeholder="例: 18000000" className="input" /></div>
          <div><label className="text-sm font-semibold mb-1" style={{ display: 'block' }}>仕入れ目標率（%）</label><input type="number" value={targetRate} onChange={e => setTargetRate(e.target.value)} placeholder="20" className="input" /></div>
        </div>
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            {bonusSettings.map(setting => {
              const targetBudget = Math.round(setting.targetSales * setting.targetRate / 100)
              const bonus = calcBonus(setting)
              const distribution = calcDistribution(setting)
              return (
                <div key={setting.id} className="border rounded p-4">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <div className="font-bold">{setting.periodStart} 〜 {setting.periodEnd}</div>
                      <div className="text-sm text-gray-500">売上目標: ¥{setting.targetSales?.toLocaleString()} / 仕入れ率: {setting.targetRate}%</div>
                    </div>
                    <button onClick={() => deletePeriod(setting.id)} className="text-red-500 text-sm">削除</button>
                  </div>
                  <div className="grid-3 gap-4 mb-4">
                    <div className="bg-blue-50 p-3 rounded text-center"><div className="text-sm text-gray-600">仕入れ目標</div><div className="text-lg font-bold text-blue-600">¥{targetBudget.toLocaleString()}</div></div>
                    <div className="bg-purple-50 p-3 rounded text-center"><div className="text-sm text-gray-600">実際の仕入れ</div><div className="text-lg font-bold text-purple-600">¥{setting.actualPurchase?.toLocaleString()}</div></div>
                    <div className={`p-3 rounded text-center ${bonus > 0 ? 'bg-green-50' : 'bg-gray-50'}`}><div className="text-sm text-gray-600">ボーナス原資</div><div className={`text-lg font-bold ${bonus > 0 ? 'text-green-600' : 'text-gray-400'}`}>¥{bonus.toLocaleString()}</div></div>
                  </div>
                  {bonus > 0 && (
                    <details>
                      ...
                      <div className="mt-4 overflow-x-auto">
                        <table className="text-sm">
                          <thead><tr><th>スタッフ</th><th className="text-center">勤続係数</th><th className="text-center">勤務係数</th><th className="text-center">特別係数</th><th className="text-center">総合係数</th><th className="text-right">配分額</th></tr></thead>
                          <tbody>
                            {distribution.map(s => (
                              <tr key={s.id}>
                                <td className="font-semibold">{s.name} {s.isOpeningStaff && <span className="badge badge-yellow text-xs">OP</span>}</td>
                                <td className="text-center">{s.tenureRate}%</td>
                                <td className="text-center">{s.workType === 'part' ? `${s.partTimeRate}%` : '100%'}</td>
                                <td className="text-center">+{s.specialRate}%</td>
                                <td className="text-center font-semibold">{(s.coef * 100).toFixed(0)}%</td>
                                <td className="text-right font-bold text-green-600">¥{s.share.toLocaleString()}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
