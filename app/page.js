...
function UsageTracking({ products, staff, usage, setUsage, stockIn, setStockIn, favorites, setFavorites }) {
  ...
  const groupedProducts = products.reduce((acc, p) => { if (!acc[p.largeCategory]) acc[p.largeCategory] = {}; if (!acc[p.largeCategory][p.mediumCategory]) acc[p.largeCategory][p.mediumCategory] = []; acc[p.largeCategory][p.mediumCategory].push(p); return acc }, {})
      ...
      {inputMode === 'single' && (
        <div className="card">
          ...
          <div className="space-y-4">
            ...
            <div><label className="text-sm font-semibold mb-2" style={{ display: 'block' }}>商品</label><select value={selectedProduct} onChange={e => setSelectedProduct(e.target.value)} className="select"><option value="">選択</option>{products.map(p => <option key={p.id} value={p.id}>{p.largeCategory} - {p.mediumCategory} - {p.name}</option>)}</select></div>
